...
describe("Loader Data Endpoint", () => {
  it("should respond with loader data", async () => {
    const res = await app.request("/api/loader-data", {}, MOCK_ENV);
  ...
  it("should be publicly accessible", async () => {
    // Loader data endpoint should be publicly accessible
    const res = await app.request("/api/loader-data", {}, MOCK_ENV);
    expect(res.status).toBe(200);
  });
});
